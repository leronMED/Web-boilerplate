...
const editUser = (user) => {
  return {
    id:
      user.id.name && user.id.value ? `${user.id.name}${user.id.value}` : null,
    gender: user.gender,
...
const mergeUsers = (randomUserMock, additionalUsers) => {
  ...
  const allUsers = [...new Set([...formattedRandomUsers, ...additionalUsers])];
...
function filterUsers(users, { country, age, gender, favorite }) {
  return users.filter((user) => {
    const countryMatch = country === undefined || user.country === country;

    const ageMatch = age === undefined || user.age === age;

    const genderMatch = gender === undefined || user.gender === gender;

    const favoriteMatch = favorite === undefined || user.favorite === favorite;

    return countryMatch && ageMatch && genderMatch && favoriteMatch;
  });
}

const filteredCountry = filterUsers(allFormattedUsers, {
  country: "Netherlands",
});
console.log(filteredCountry);
const filteredAge = filterUsers(allFormattedUsers, {
  age: 27,
});
console.log(filteredAge);
const filteredGender = filterUsers(allFormattedUsers, {
  gender: "male",
});
console.log(filteredGender);
const filteredFavorite = filterUsers(allFormattedUsers, {
  favorite: false,
});
