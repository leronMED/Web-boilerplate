...
// FIRST TASK
console.log(additionalUsers.length);
const courses = [
  "Mathematics",
  "Physics",
  "English",
...
const editUser = (user) => {
  const generatedId = () => {
    // Генерація унікального id на основі ім'я, прізвище та email користувача
  ...
  return {
    id:
      user.id && user.id.name && user.id.value
        ...
        : generatedId(), // Генеруємо id, якщо відсутнє
    gender: user.gender,
...
const mergeUsers = (randomUserMock, additionalUsers) => {
  const formattedRandomUsers = randomUserMock.map(editUser);

  const allUsers = [];

  const addUniqueUser = (user) => {
  ...
  formattedRandomUsers.forEach(addUniqueUser);

  additionalUsers.forEach(addUniqueUser);
...
const allFormattedUsers = mergeUsers(randomUserMock, additionalUsers);
console.log(allFormattedUsers);
...
function validateUser(user) {
  ...
  if (typeof user.age !== "number" || isNaN(user.age)) {
    errors.push("Age failed validation");
  }

  const phoneRegex = /^\+380\d{9}$/;
  if (!phoneRegex.test(user.phone)) {
    errors.push("Phone failed validation");
  }

  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(user.email)) {
    errors.push("Email failed validation.");
  }

  if (errors.length > 0) {
    return { valid: false, errors: errors };
  } else {
    return { valid: true, message: "passed validation" };
  }
}

console.log(allFormattedUsers.map(validateUser));
...
// const people = [
//   { full_name: "Darrell Dunne", age: 35, country: "Ireland" },
//   { full_name: "Matilda Aalto", age: 29, country: "Finland" },
//   { full_name: "Viktor Legrand", age: 40, country: "France" },
// ];

console.log(sortArray(allFormattedUsers, "b_date"));

console.log(sortArray(allFormattedUsers, "age"));

console.log(sortArray(allFormattedUsers, "full_name"));
// TASK 5
...
const searchByName = findAllObjectsByParam(
  allFormattedUsers,
  "full_name",
  "sara"
);
...
const searchByAge = findAllObjectsByParam(allFormattedUsers, "age", 27);
...
function calculatePercentage(array, searchField, searchValue) {
  function getComparisonOperator(value) {
    ...
    if (value.startsWith(">=")) {
      operator = (a) => a >= number;
      number = parseFloat(value.slice(2));
  ...
  const { operator, number } = getComparisonOperator(searchValue);

  const filtered = array.filter((item) => {
    const value = item[searchField];
    return typeof value === "number" && operator(value);
  });

  const percentage = (filtered.length / array.length) * 100;

  return percentage.toFixed(2);
}

console.log(calculatePercentage(allFormattedUsers, "age", ">30"));
console.log(calculatePercentage(allFormattedUsers, "age", "<=30"));
console.log(calculatePercentage(allFormattedUsers, "age", "30"));
console.log(calculatePercentage(allFormattedUsers, "age", "<40"));
...
let currentUsers = allFormattedUsers;
...
function createSortableTable(allFormattedUsers) {
  ...
  headers.forEach((header) => {
    ...
    th.addEventListener("click", () => {
      allFormattedUsers = sortArray(allFormattedUsers, header.field, ascending);
      updateTableBody(tbody, allFormattedUsers);
      ascending = !ascending;
    });
...
createSortableTable(allFormattedUsers);
...
form.addEventListener("submit", (e) => {
  ...
  const user = {
    ...
    age: calculateAge(form.querySelector("#date").value),
...
function displayUsersFavorite() {
  ...
  // Limit the displayed avatars to maxVisibleAvatars
  ...
  // Disable/enable arrows based on current view
